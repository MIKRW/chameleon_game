...
// bolted into the bark on the left face of a trunk near its top — the
// second tree from the far right (see LIGHT_SWITCH_PLACEMENT,
// world-props.js) — deliberately so flipping it means climbing across the
// tank and up, rather than something reachable by just poking around near
// the bulb/background-texture. Toggling it (see
...
// right (columns 10-15), its tip embedding into the trunk bark the same
// way the old vertical bracket did. The square's interior is solid black
// (k) here for off, and glows red (z, light-switch-2.js) for on — the
// brass (a/A) frame and arm don't change between states.
//
